...
$sql = "SELECT cid FROM zw_cart WHERE userId='$userId'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_row($result);

if($row===null){	//没有查询到购物车编号
	//SQL2：如果用户没有购物车，则添加一个购物车，获得购物车编号
	$sql = "INSERT INTO zw_cart VALUES(NULL,'$userId')";
...
$sql = "SELECT did,count FROM zw_cart_detail WHERE cartId='$cartId' AND productId='$productId'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_row($result);

if($row===null){  //指定购物车中没有该商品的购买记录
	//SQL4：若之前未购买过该商品，则插入一条购买记录，购买数量为1
	$sql = "INSERT INTO zw_cart_detail VALUES(NULL,'$cartId','$productId','1')";
	mysqli_query($conn,$sql);
	$count = 1;
}else {   //指定购物车中买过该商品	
	...
	$count = $row[1];	//购买数量
	$count++;
	$sql = "UPDATE zw_cart_detail SET count='$count' WHERE did='$did'";
	mysqli_query($conn,$sql);
}
